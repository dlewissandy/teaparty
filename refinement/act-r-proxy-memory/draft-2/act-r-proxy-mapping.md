...
Note that because normalization is computed over the candidate set, the effective influence of the activation_weight / semantic_weight balance varies by query. When all candidates have similar activation, semantic relevance dominates — which is the correct behavior (equally accessible chunks should be ranked by relevance). When activation varies widely, it has strong discriminating power. The 0.5 / 0.5 starting point is a design parameter whose effective behavior is query-dependent.
