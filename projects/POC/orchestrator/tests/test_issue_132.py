...
class TestStaticRetrieveMemoryToggles(unittest.TestCase):
    """_retrieve_memory_static respects the same toggles as the instance method."""

    def test_static_skip_returns_empty(self):
        td = _make_project_dir_with_memory()
        with td:
            result = Session._retrieve_memory_static(
                'test task', '/tmp/poc', td.name,
                skip_learning_retrieval=True,
            )
            self.assertEqual(result, '')

    def test_static_disabled_returns_empty(self):
        td = _make_project_dir_with_memory()
        with td:
            result = Session._retrieve_memory_static(
                'test task', '/tmp/poc', td.name,
                learning_retrieval_mode='disabled',
            )
            self.assertEqual(result, '')

    def test_static_flat_returns_content(self):
        td = _make_project_dir_with_memory()
        with td:
            result = Session._retrieve_memory_static(
                'test task', '/tmp/poc', td.name,
                learning_retrieval_mode='flat',
            )
            self.assertIn('Institutional', result)

    def test_static_scoped_passes_scope_base_dir(self):
        td = _make_project_dir_with_memory()
        with td:
            db_path = os.path.join(td.name, '.memory.db')
            Path(db_path).touch()
            ...
            with patch('projects.POC.scripts.memory_indexer.retrieve', return_value='') as mock:
                Session._retrieve_memory_static(
                    'test task', '/tmp/poc', td.name,
                    learning_retrieval_mode='scoped',
                )
                mock.assert_called_once()
                _, kwargs = mock.call_args
                self.assertEqual(kwargs['scope_base_dir'], td.name)

    def test_static_flat_passes_empty_scope_base_dir(self):
        td = _make_project_dir_with_memory()
        with td:
            db_path = os.path.join(td.name, '.memory.db')
            Path(db_path).touch()
