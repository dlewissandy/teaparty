...
def _standard_patches(dispatch_infra, mock_result, squash_merge_side_effect=None,
    ...
    return {
        'config_cls': patch('projects.POC.orchestrator.dispatch_cli.PhaseConfig'),
        'create_wt': patch('projects.POC.orchestrator.dispatch_cli.create_dispatch_worktree',
                           new=AsyncMock(return_value=mock_dispatch_info)),
        'cleanup_wt': patch('projects.POC.orchestrator.dispatch_cli.cleanup_worktree',
                            new=AsyncMock()),
        'squash': patch('projects.POC.orchestrator.dispatch_cli.squash_merge',
                        new=squash_mock),
        'orch_cls': patch('projects.POC.orchestrator.dispatch_cli.Orchestrator'),
        'generate': patch('projects.POC.orchestrator.dispatch_cli.generate_async',
                          new=generate_mock),
        'build_fb': patch('projects.POC.orchestrator.dispatch_cli.build_fallback',
                          return_value='fallback msg'),
        'git_output': patch('projects.POC.orchestrator.dispatch_cli.git_output',
...
class TestDispatchMergeFailureSkipsMemory(unittest.TestCase):
    ...
    def test_memory_not_written_on_merge_failure(self):
        ...
        with patch('projects.POC.orchestrator.dispatch_cli._write_dispatch_memory') as mock_mem:
            result = _run_dispatch_with_patches(self.tmpdir, self.dispatch_infra, patches)
...
class TestDispatchCleanupAfterMergeFailure(unittest.TestCase):
    ...
    def test_cleanup_worktree_called_on_merge_failure(self):
        ...
        with patch('projects.POC.orchestrator.dispatch_cli.PhaseConfig') as cfg_cls, \
             ...
             patch('projects.POC.orchestrator.dispatch_cli.Orchestrator') as orch_cls, \
             patch('projects.POC.orchestrator.dispatch_cli.generate_async',
                   new=AsyncMock(return_value='msg')), \
             patch('projects.POC.orchestrator.dispatch_cli.build_fallback',
