...
class TestExtractLearningsMustNotBlockEventLoop(unittest.TestCase):
    ...
    def test_concurrent_task_gets_ticks_during_extraction(self):
        ...
        async def run_test():
            ...
            with patch('projects.POC.orchestrator.learnings._run_summarize') as mock_sum, \
                 patch('projects.POC.orchestrator.learnings._promote_team') as m1, \
                 patch('projects.POC.orchestrator.learnings._promote_session') as m2, \
                 patch('projects.POC.orchestrator.learnings._promote_project') as m3, \
                 patch('projects.POC.orchestrator.learnings._promote_global') as m4, \
                 patch('projects.POC.orchestrator.learnings._promote_prospective') as m5, \
                 patch('projects.POC.orchestrator.learnings._promote_in_flight') as m6, \
                 patch('projects.POC.orchestrator.learnings._promote_corrective') as m7:
