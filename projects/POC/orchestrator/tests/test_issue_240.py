...
from projects.POC.orchestrator.phase_config import PhaseConfig, TeamSpec
...
class TestDispatchSkipsWorktreeForDirect(unittest.TestCase):
    ...
    def test_direct_model_does_not_call_create_dispatch_worktree(self):
        ...
        with patch('projects.POC.orchestrator.dispatch_cli.create_dispatch_worktree') as mock_create, \
             patch('projects.POC.orchestrator.dispatch_cli.Orchestrator') as mock_orch, \
             patch('projects.POC.orchestrator.dispatch_cli.cleanup_worktree', new_callable=AsyncMock), \
             patch('projects.POC.orchestrator.dispatch_cli.load_state', return_value=MagicMock()), \
             patch('projects.POC.orchestrator.dispatch_cli.make_child_state', return_value=MagicMock()), \
             patch('projects.POC.orchestrator.dispatch_cli.save_state'), \
             patch('projects.POC.orchestrator.heartbeat.register_child', return_value=None), \
             patch('projects.POC.orchestrator.heartbeat.finalize_heartbeat', return_value=None):
...
class TestDispatchSkipsMergeForDirect(unittest.TestCase):
    ...
    def test_direct_model_does_not_call_squash_merge(self):
        ...
        with patch('projects.POC.orchestrator.dispatch_cli.create_dispatch_worktree') as mock_create, \
             patch('projects.POC.orchestrator.dispatch_cli.squash_merge') as mock_merge, \
             patch('projects.POC.orchestrator.dispatch_cli.Orchestrator') as mock_orch, \
             patch('projects.POC.orchestrator.dispatch_cli.cleanup_worktree', new_callable=AsyncMock), \
             patch('projects.POC.orchestrator.dispatch_cli.load_state', return_value=MagicMock()), \
             patch('projects.POC.orchestrator.dispatch_cli.make_child_state', return_value=MagicMock()), \
