...
class TestSessionCleansUpWorktree(unittest.TestCase):
    ...
    def test_session_calls_cleanup_worktree_on_completed_work(self):
        ...
        with patch('projects.POC.orchestrator.session.create_session_worktree',
                   new=AsyncMock(return_value=session_info)), \
             patch('projects.POC.orchestrator.session.cleanup_worktree',
                   new=cleanup_mock), \
             patch('projects.POC.orchestrator.session.commit_deliverables',
                   new=AsyncMock(return_value='abc123')), \
             patch('projects.POC.orchestrator.session.squash_merge',
                   new=AsyncMock()), \
             patch('projects.POC.orchestrator.session.extract_learnings',
                   new=AsyncMock()), \
             ...
             patch('projects.POC.orchestrator.session.Orchestrator') as mock_orch_cls, \
             patch('projects.POC.orchestrator.session.StateWriter') as mock_sw_cls, \
             patch('projects.POC.orchestrator.session.PhaseConfig'):
