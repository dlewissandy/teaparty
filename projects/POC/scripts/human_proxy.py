#!/usr/bin/env python3
"""Human Proxy — confidence-based proxy with content-awareness + conversation-learning.
...
Content-awareness: reads the artifact under review and checks for length
anomalies and correction patterns before making the confidence decision.

Conversation-learning: stores full review conversations alongside text
differentials so that correction patterns are learned from dialog, not just
outcome summaries.
...
Usage:
    ...
    # Record — human corrected the output
    python3 human_proxy.py --record --state PLAN_ASSERT --task-type my-project \\
        --outcome correct --diff "missing rollback handling" \\
        --artifact /path/to/plan.md \\
        --conversation "Human: where is rollback? Agent: not included." \\
        --model /path/to/.proxy-confidence.json
...
# Phase 2 — minimum times a token must appear in correction history to be "expected".
...
class ConfidenceEntry:
    ...
    ema_approval_rate: float = None  # exponential moving average — recency-weighted approval rate; None = bootstrap from Laplace
    artifact_lengths: list = field(default_factory=list)   # [int, ...] char counts of reviewed artifacts
    last_approved_length: int = 0   # char count of the most recently approved artifact
    conversations: list = field(default_factory=list)  # [str, ...] full review conversation texts
...
def _check_content_novelty(artifact_text: str, entry: ConfidenceEntry) -> tuple:
    ...
    # Differentials AND conversation texts are co-equal inputs
...
def should_escalate(
    ...
    2. Content check (Phase 1 / Phase 2) — fires regardless of confidence level.
    3. Choose threshold based on whether the state is generative or binary.
    4. If confidence >= threshold → auto-approve; otherwise → escalate.
    """
    ...
    if raw is None:
        ...
        if 'last_approved_length' not in raw:
            raw['last_approved_length'] = 0
...
def record_outcome(
    ...
    artifact_length (optional): char count of the artifact under review.
    conversation_text (optional): full text of the review conversation (for non-approve).
    ...
    if raw is None:
        ...
        entry = ConfidenceEntry(
            ...
            last_approved_length=getattr(raw, 'last_approved_length', 0),
            conversations=list(getattr(raw, 'conversations', [])),
        )
    ...
    if ema is None:
        # Bootstrap from Laplace before first EMA update
        prev_tc = entry.total_count  # before incrementing
        ema = (entry.approve_count + 1) / (prev_tc + 2) if prev_tc > 0 else 0.5
    if outcome == 'approve':
    ...
    last_approved_length = entry.last_approved_length if hasattr(entry, 'last_approved_length') else 0
    if outcome == 'approve' and artifact_length > 0:
        last_approved_length = artifact_length

    # Track conversation text for correction pattern learning
    conversations = list(getattr(entry, 'conversations', []))
    if conversation_text and outcome != 'approve':
        conversations.append(conversation_text[:2000])
        if len(conversations) > MAX_CONVERSATIONS_PER_ENTRY:
            conversations = conversations[-MAX_CONVERSATIONS_PER_ENTRY:]
    ...
    updated_entry = ConfidenceEntry(
        ...
        last_approved_length=last_approved_length,
...
def _cmd_record(args) -> None:
    ...
    if artifact_path:
        ...
        artifact_len = len(content)
    conversation_text = getattr(args, 'conversation', '') or ''
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Human Proxy — confidence-based CfA approval proxy with content-awareness"
    )
    ...
    parser.add_argument('--diff', default='',
                        help="Text differential summary — what the human changed (for --record)")
    parser.add_argument('--artifact', default='',
                        help='Path to the artifact under review (for --decide and --record)')
    parser.add_argument('--conversation', default='',
                        help='Full text of the review conversation (for --record)')
