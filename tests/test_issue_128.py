...
from scripts.summarize_session import extract_human_turns, promote
from orchestrator.learnings import extract_learnings
from orchestrator.events import EventBus, EventType, Event
...
class TestSessionCreatesTeamInfraDirs(unittest.TestCase):
    ...
    def test_session_creates_team_dirs_in_infra(self):
        ...
        """
        from orchestrator.worktree import create_session_worktree

        async def _test():
            with patch('orchestrator.worktree._run_git',
                       new=AsyncMock()), \
                 patch('orchestrator.worktree._register_worktree'):
...
class TestDispatchWritesMemory(unittest.TestCase):
    ...
    def test_dispatch_writes_memory_md_after_completion(self):
        ...
        from orchestrator.engine import OrchestratorResult
        ...
        with patch.dict(os.environ, {
            ...
            patch('orchestrator.dispatch_cli.load_state',
                  return_value={'state': 'TASK', 'task_id': 'p', 'history': [],
                                'phase': 'execution', 'backtrack_count': 0}), \
            patch('orchestrator.dispatch_cli.make_child_state',
                  return_value={'state': 'TASK', 'task_id': 'c', 'history': [],
                                'phase': 'execution', 'backtrack_count': 0}), \
            patch('orchestrator.dispatch_cli.save_state'), \
            patch('orchestrator.dispatch_cli.Orchestrator',
...
class TestRollupChainEndToEnd(unittest.TestCase):
    ...
    def test_rollup_chain_produces_output_with_dispatch_memories(self):
        ...
        with patch('scripts.summarize_session.summarize',
