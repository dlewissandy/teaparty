...
class TestConsultProxyIsTheOnePath(unittest.TestCase):
    ...
    def test_approval_gate_uses_consult_proxy(self):
        ...
        try:
            ...
            with patch('orchestrator.proxy_agent.consult_proxy', new_callable=AsyncMock) as mock_cp, \
...
class TestAgentAlwaysInvoked(unittest.TestCase):
    ...
    def test_agent_invoked_on_cold_start(self):
        ...
        with patch('orchestrator.proxy_agent.run_proxy_agent', mock_agent):
...
class TestStatisticalCalibration(unittest.TestCase):
    ...
    def test_deep_memory_passes_agent_confidence_through(self):
        ...
        with patch('orchestrator.proxy_agent.run_proxy_agent', mock_agent), \
             patch('orchestrator.proxy_agent._get_memory_depth', return_value=10), \
    ...
    def test_ema_does_not_reduce_agent_confidence(self):
        ...
        with patch('orchestrator.proxy_agent.run_proxy_agent', mock_agent), \
             patch('orchestrator.proxy_agent._get_memory_depth', return_value=10), \
             patch('orchestrator.proxy_agent.random') as mock_rng:
