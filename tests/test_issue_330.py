...
def _make_management_team(tmpdir: str, agents=None, hooks=None, scheduled=None):
    """Write teaparty.yaml into tmpdir/.teaparty/management/ and load the management team."""
...
class TestManagementTeamAgentsHaveFilePath(unittest.TestCase):
    ...
    def test_agent_file_points_to_agent_md(self):
        """Agent 'file' must be absolute path to agents/{name}/agent.md when definition exists."""
        agents_dir = os.path.join(self.tmp, '.teaparty', 'management', 'agents')
        ...
        for a in result['agents']:
            expected = os.path.join(self.tmp, '.teaparty', 'management', 'agents', a['name'], 'agent.md')
...
class TestManagementTeamSkillsHaveFilePath(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Create real skills at management skills dir
        org_skills_dir = os.path.join(self.tmp, '.teaparty', 'management', 'skills')
    ...
    def test_skill_file_points_to_skill_md(self):
        """Skill 'file' must be absolute path to management skills/{name}/SKILL.md."""
        org_skills_dir = os.path.join(self.tmp, '.teaparty', 'management', 'skills')
        from orchestrator.config_reader import discover_skills
        discovered = discover_skills(org_skills_dir)
        result = self.bridge._serialize_management_team(
        ...
        for s in result['skills']:
            expected = os.path.join(self.tmp, '.teaparty', 'management', 'skills', s['name'], 'SKILL.md')
...
class TestProjectTeamSkillsHaveFilePath(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Create org skill at management skills dir
        org_skills_dir = os.path.join(self.tmp, '.teaparty', 'management', 'skills')
        _make_skill(org_skills_dir, 'audit')
        self.team = _make_project_team(self.project_dir, skills=['audit'])

    def test_local_skill_file_resolves_from_project_dir(self):
        """A local skill's 'file' resolves under project_dir/.teaparty/project/skills/{name}/SKILL.md."""
        result = self.bridge._serialize_project_team(
            self.team,
            org_agents=[],
        ...
        expected = os.path.join(self.project_dir, '.teaparty', 'project', 'skills', 'local-skill', 'SKILL.md')
        self.assertEqual(skill['file'], expected)

    def test_shared_skill_file_resolves_from_teaparty_home(self):
        """A shared (org) skill's 'file' resolves under management skills/{name}/SKILL.md."""
        result = self.bridge._serialize_project_team(
            self.team,
            org_agents=[],
            local_skills=['local-skill'],
        ...
        expected = os.path.join(self.tmp, '.teaparty', 'management', 'skills', 'audit', 'SKILL.md')
        self.assertEqual(skill['file'], expected)
...
class TestManagementTeamHooksHaveFilePath(unittest.TestCase):
    ...
    def test_hook_without_command_falls_back_to_teaparty_yaml(self):
        """A hook with no command field gets file = management/teaparty.yaml."""
        ...
        expected = os.path.join(self.teaparty_home, 'management', 'teaparty.yaml')
        self.assertEqual(h['file'], expected)

    def test_hook_with_nonfile_command_falls_back_to_teaparty_yaml(self):
        """A hook whose command is a shell string (not a file) gets file = management/teaparty.yaml."""
        hooks = [{'event': 'PreToolUse', 'matcher': 'Bash', 'type': 'command',
                  'command': 'echo hello'}]
        team = _make_management_team(self.tmp, hooks=hooks)
        result = self.bridge._serialize_management_team(
...
class TestProjectTeamHooksHaveFilePath(unittest.TestCase):
    ...
    def test_hook_without_command_falls_back_to_project_yaml(self):
        ...
        h = result['hooks'][0]
        self.assertIn('file', h, 'Hook entry must have a "file" key')
        expected = os.path.join(self.project_dir, '.teaparty', 'project', 'project.yaml')
        self.assertEqual(h['file'], expected)
...
class TestManagementTeamScheduledTasksHaveFilePath(unittest.TestCase):
    ...
    def test_scheduled_task_file_points_to_skill_md(self):
        """Scheduled task 'file' must point to the skill's SKILL.md at management skills/."""
        scheduled = [{'name': 'nightly', 'schedule': '0 2 * * *', 'skill': 'audit', 'enabled': True}]
        team = _make_management_team(self.tmp, scheduled=scheduled)
        result = self.bridge._serialize_management_team(
            team, discovered_skills=[], teaparty_home=self.teaparty_home
...
class TestProjectTeamScheduledTasksHaveFilePath(unittest.TestCase):
    ...
    def test_project_scheduled_task_file_resolves_from_local_skills_first(self):
        ...
        task = result['scheduled'][0]
        expected = os.path.join(self.project_dir, '.teaparty', 'project', 'skills', 'local-audit', 'SKILL.md')
        self.assertEqual(task['file'], expected)


# ── Criterion 9: discover_hooks reads Claude Code settings.json ───────────────

def _write_settings_yaml(path: str, hooks: dict) -> None:
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, 'w') as f:
        yaml.dump({'hooks': hooks}, f)


class TestDiscoverHooks(unittest.TestCase):
    """discover_hooks must parse settings.yaml into flat hook dicts."""

    def setUp(self):
        self.tmp = tempfile.mkdtemp()
        self.settings = os.path.join(self.tmp, '.teaparty', 'management', 'settings.yaml')

    def test_returns_empty_list_when_file_absent(self):
        from orchestrator.config_reader import discover_hooks
        result = discover_hooks('/nonexistent/settings.yaml')
        self.assertEqual(result, [])

    def test_returns_empty_list_for_empty_hooks_section(self):
        from orchestrator.config_reader import discover_hooks
        _write_settings_yaml(self.settings, {})
...
class TestManagementTeamSettingsYamlHooks(unittest.TestCase):
    """_serialize_management_team must include hooks from settings.yaml."""

    def setUp(self):
        self.tmp = tempfile.mkdtemp()
        self.teaparty_home = os.path.join(self.tmp, '.teaparty')
        self.bridge = _make_bridge(self.tmp)
        # settings.yaml at management level
        self.settings_yaml = os.path.join(self.tmp, '.teaparty', 'management', 'settings.yaml')
        _write_settings_yaml(self.settings_yaml, {
            'PreToolUse': [
                {'matcher': 'Edit|Write', 'hooks': [
                    {'type': 'command', 'command': '.teaparty/hooks/enforce-ownership.sh'}
                ]}
            ]
        })
        # Create the hook script so _hook_file resolves it
        hook_script = os.path.join(self.tmp, '.teaparty', 'hooks', 'enforce-ownership.sh')
        os.makedirs(os.path.dirname(hook_script), exist_ok=True)
        with open(hook_script, 'w') as f:
            f.write('#!/bin/sh\n')

    def test_settings_hooks_appear_in_hooks_list(self):
        """Hooks from settings.yaml must appear in serialized hooks."""
    ...
    def test_settings_hooks_combined_with_yaml_hooks(self):
        """YAML hooks and settings.yaml hooks must both appear."""
    ...
    def test_settings_hook_file_resolves_to_script(self):
        """settings.yaml hook command that is a relative path resolves to absolute."""
        team = _make_management_team(self.tmp, hooks=[])
        result = self.bridge._serialize_management_team(
            team, discovered_skills=[], teaparty_home=self.teaparty_home
        )
        h = result['hooks'][0]
        expected = os.path.join(self.tmp, '.teaparty', 'hooks', 'enforce-ownership.sh')
        self.assertEqual(h['file'], expected)


# ── Criterion 11: Project team merges settings.json hooks ────────────────────

class TestProjectTeamSettingsYamlHooks(unittest.TestCase):
    """_serialize_project_team must include hooks from project settings.yaml."""
    ...
    def setUp(self):
        ...
        os.makedirs(self.project_dir)
        self.bridge = _make_bridge(self.tmp)
        # settings.yaml inside the project
        self.settings_yaml = os.path.join(self.project_dir, '.teaparty', 'project', 'settings.yaml')
        _write_settings_yaml(self.settings_yaml, {
            'PreToolUse': [
                {'matcher': 'Write', 'hooks': [
                    {'type': 'command', 'command': '.teaparty/hooks/project-check.sh'}
                ]}
            ]
        })
        hook_script = os.path.join(self.project_dir, '.teaparty', 'hooks', 'project-check.sh')
        os.makedirs(os.path.dirname(hook_script), exist_ok=True)
        with open(hook_script, 'w') as f:
            f.write('#!/bin/sh\n')

    def test_settings_hooks_appear_in_project_hooks(self):
        """Hooks from project settings.yaml must appear in serialized project hooks."""
    ...
    def test_settings_hook_file_resolves_to_project_script(self):
        ...
        h = result['hooks'][0]
        expected = os.path.join(self.project_dir, '.teaparty', 'hooks', 'project-check.sh')
        self.assertEqual(h['file'], expected)
