...
class TestDispatchCreatesTaskConversation(unittest.TestCase):
    ...
    def test_dispatch_listener_creates_task_conversation(self):
        """DispatchListener._handle_dispatch must create a TASK conversation in the session bus."""
        import asyncio
        ...
        # Patch dispatch() so we don't actually run an orchestration
        fake_result = {
            'status': 'completed',
        ...
        with patch('orchestrator.dispatch_listener.dispatch', new=AsyncMock(return_value=fake_result)), \
            ...
            asyncio.run(listener._handle_dispatch('writing', 'Write a poem'))

        bus = SqliteMessageBus(bus_path)
        task_convs = bus.active_conversations(ConversationType.TASK)
        self.assertGreater(len(task_convs), 0, 'Expected at least one TASK conversation created by dispatch')
        task_conv_id = task_convs[0].id
        self.assertTrue(task_conv_id.startswith('task:'), f'TASK conv ID must have task: prefix: {task_conv_id}')
...
class TestDispatchTurnCostWritesToTaskConversation(unittest.TestCase):
    ...
    def test_dispatch_cost_writes_cost_sender_to_task_conversation(self):
        """When dispatch completes with cost data, cost messages must appear in the task conversation.

        The dispatch result includes total_cost_usd from the dispatch orchestrator's cost sidecar.
        The listener writes this as a cost-sender message to the TASK conversation.
        """
        import asyncio
        from unittest.mock import AsyncMock, patch
        from orchestrator.messaging import SqliteMessageBus, ConversationType
        ...
        # Dispatch result includes total_cost_usd (extracted from dispatch sidecar by dispatch_cli)
        fake_result = {
            'status': 'completed',
            'terminal_state': 'COMPLETED_WORK',
            'team': 'writing',
        ...
        bus = SqliteMessageBus(bus_path)
        task_convs = bus.active_conversations(ConversationType.TASK)
        self.assertGreater(len(task_convs), 0, 'Expected task conversation to be created')

        task_conv_id = task_convs[0].id
        messages = bus.receive(task_conv_id, since_timestamp=0.0)
        cost_msgs = [m for m in messages if m.sender == 'cost']
        self.assertGreater(len(cost_msgs), 0,
                           'Expected at least one cost sender message in task conversation')

    def test_dispatch_cost_message_has_total_cost_usd(self):
        """Cost messages in task conversation must include total_cost_usd."""
        import asyncio
        from unittest.mock import AsyncMock, patch
        from orchestrator.messaging import SqliteMessageBus, ConversationType
        ...
        messages = bus.receive(task_conv_id, since_timestamp=0.0)
        cost_msgs = [m for m in messages if m.sender == 'cost']
        self.assertGreater(len(cost_msgs), 0, 'Expected cost message in task conversation')
        stats = json.loads(cost_msgs[0].content)
