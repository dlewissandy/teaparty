...
class _TestBridge(TeaPartyBridge):
    ...
    """
    def __init__(self, teaparty_home: str, static_dir: str, projects_dir: str | None = None):
        super().__init__(teaparty_home=teaparty_home, static_dir=static_dir)
        self._test_projects_dir = projects_dir
        if projects_dir is not None:
            # Override state reader to use directory-scan mode for test fixtures.
            # Tests write worktrees.json to teaparty_home (= repo_root in test context).
            from orchestrator.state_reader import StateReader
            self._state_reader = StateReader(
                repo_root=teaparty_home,
                projects_dir=projects_dir,
            )

    def _lookup_project_path(self, slug: str) -> str | None:
        if self._test_projects_dir:
            path = os.path.join(self._test_projects_dir, slug)
            return path if os.path.isdir(path) else None
        return super()._lookup_project_path(slug)

    def _resolve_session_infra(self, session_id: str) -> str | None:
        if self._test_projects_dir:
            for slug in os.listdir(self._test_projects_dir):
                candidate = os.path.join(self._test_projects_dir, slug, '.sessions', session_id)
                if os.path.isdir(candidate):
                    return candidate
            return None
        return super()._resolve_session_infra(session_id)
