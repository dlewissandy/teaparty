...
# ── Layer 9: Socratic querying at INTENT_ASSERT ───────────────────────────────

class TestSocraticQuerying(unittest.TestCase):
    """At INTENT_ASSERT with artifact_missing=True, ApprovalGate must skip the proxy gate
    and wait for the human's next reply, returning ActorResult(action='correct').

    This enables the Socratic querying loop: agent asks questions → human replies →
    agent tries again → repeat until INTENT.md is produced.  The state machine's
    INTENT_ASSERT → correct → INTENT_RESPONSE → INTENT_RUN cycle drives the iteration.
    """

    def _make_approval_gate(self, input_provider):
        """Return a minimal ApprovalGate with a stub input_provider."""
    ...
    def _make_context(self, state: str, artifact_missing: bool) -> 'ActorContext':
        from teaparty.cfa.actors import ActorContext
        from teaparty.cfa.phase_config import PhaseSpec
        from teaparty.messaging.bus import EventBus
        ...
        return ActorContext(
            state=state,
            phase='intent',
            task='Create a list of user stories.',
            infra_dir=tempfile.mkdtemp(),
            project_workdir=tempfile.mkdtemp(),
            session_worktree=tempfile.mkdtemp(),
            stream_file='intent.jsonl',
            phase_spec=phase_spec,
            poc_root=tempfile.mkdtemp(),
            event_bus=EventBus(),
            data={'artifact_missing': artifact_missing, 'artifact_expected': 'INTENT.md'},
        )

    def test_intent_assert_with_missing_artifact_takes_socratic_path(self):
        """At INTENT_ASSERT+artifact_missing=True, gate must await input_provider and return 'correct'."""
        human_reply_text = 'I want the stories focused on mobile users'
        input_provider_called = []

        async def stub_input_provider(request):
            input_provider_called.append(request)
            return human_reply_text

        gate = self._make_approval_gate(stub_input_provider)
        ctx = self._make_context(state='INTENT_ASSERT', artifact_missing=True)

        result = _run(gate.run(ctx))

        self.assertEqual(len(input_provider_called), 1,
                         'input_provider must be called exactly once on the Socratic path')
        self.assertEqual(
            result.action, 'correct',
            f"Socratic path must return action='correct', got '{result.action}'; "
            "state machine routes this to INTENT_RESPONSE so the agent gets another turn",
        )
        self.assertEqual(
            result.feedback, human_reply_text,
            f"feedback must carry the human's reply verbatim ('{human_reply_text}'), "
            f"got '{result.feedback}'",
        )

    def test_intent_assert_empty_bridge_text_sends_no_message(self):
        """Socratic path calls input_provider with empty bridge_text — no spurious message in bus."""
        bus, db_path = _make_bus(self)
        conv_id = make_conversation_id(ConversationType.JOB, 'tp:socratic-001')
        bus.create_conversation(ConversationType.JOB, 'tp:socratic-001')

        provider = MessageBusInputProvider(
            bus=bus,
            conversation_id=conv_id,
            sender='teaparty-lead',
            poll_interval=0.01,
        )
        ...
        async def run_provider():
            async def inject_reply():
                for _ in range(200):
                    await asyncio.sleep(0.01)
                    if bus.get_conversation(conv_id).awaiting_input:
                        break
                bus.send(conv_id, 'human', 'Focus on mobile users')

            request = InputRequest(type='input', state='INTENT_ASSERT', bridge_text='')
            task = asyncio.create_task(inject_reply())
            result = await provider(request)
            await task
            return result
        ...
        non_human = [m for m in bus.receive(conv_id) if m.sender != 'human']
        self.assertEqual(
            len(non_human), 0,
            f"empty bridge_text must produce no non-human message in the bus; "
            f"got {len(non_human)}: {[(m.sender, m.content) for m in non_human]}",
        )

    def test_non_intent_assert_state_does_not_take_socratic_path(self):
        """At PLAN_ASSERT, artifact_missing=True must NOT take the Socratic shortcut."""
        from teaparty.cfa.actors import ApprovalGate

        socratic_input_called = []

        async def stub_input_provider(request):
            socratic_input_called.append(request)
            return 'some reply'

        gate = self._make_approval_gate(stub_input_provider)
        ctx = self._make_context(state='PLAN_ASSERT', artifact_missing=True)

        # Patch _ask_human_through_proxy so we can detect that the normal gate path
        # was taken without running real proxy machinery.
        class _SentinelError(Exception):
            pass

        with patch.object(gate, '_ask_human_through_proxy', side_effect=_SentinelError()):
            with self.assertRaises(_SentinelError):
                _run(gate.run(ctx))

        self.assertEqual(
            len(socratic_input_called), 0,
            "PLAN_ASSERT must not trigger the Socratic shortcut — "
            "input_provider must not be called before the normal gate loop runs",
        )
