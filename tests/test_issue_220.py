...
from tests.test_issue_179 import _make_chunk
...
def _calibrate_with_mocks(
    ...
    *,
    ...
    with patch('scripts.approval_gate.resolve_team_model_path',
               return_value='/tmp/test.json'), \
         patch('scripts.approval_gate.load_model',
               return_value=mock_model), \
         patch('scripts.approval_gate._entry_key',
               return_value='PLAN_ASSERT::test'), \
         patch('scripts.approval_gate._make_entry',
               return_value=mock_entry), \
         patch('scripts.approval_gate.compute_confidence_components',
               return_value=(laplace, ema)), \
         patch('orchestrator.proxy_agent._get_memory_depth',
