...
from orchestrator.state_reader import SessionState, DispatchState
from tests.test_helpers import make_tmp_dir
...
class TestWithdrawEventPublishing(unittest.TestCase):
    ...
    def test_withdraw_event_before_kill(self):
        ...
        session = _make_session_state(self.infra_dir)
        with patch('orchestrator.withdraw._kill_pid', side_effect=tracking_kill):
            _run(withdraw_session(session, event_bus=bus))
...
class TestCascadingProcessTermination(unittest.TestCase):
    ...
    def test_kills_session_pid(self):
        ...
        session = _make_session_state(self.infra_dir)
        with patch('orchestrator.withdraw._kill_pid') as mock_kill:
...
class TestGuardRails(unittest.TestCase):
    ...
    def test_depth_bounded_recursion(self):
        ...
        from orchestrator.withdraw import _kill_nested_dispatches
        ...
        killed = []
        with patch('orchestrator.withdraw._kill_pid', side_effect=lambda pid: killed.append(pid)):
