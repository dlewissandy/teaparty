...
class TestGetAccuracy(unittest.TestCase):
    ...
    def test_get_accuracy_returns_record(self):
        from orchestrator.proxy_memory import get_accuracy
        conn, _ = _make_db()
        try:
            record_interaction(
                conn,
...
class TestAccuracyInConsultProxy(unittest.TestCase):
    ...
    def test_accuracy_data_passed_to_proxy_agent(self):
        ...
        with patch('orchestrator.proxy_agent._invoke_claude_proxy', side_effect=_fake_invoke), \
             patch('orchestrator.proxy_agent._retrieve_actr_memories') as mock_retrieve, \
             patch('orchestrator.proxy_agent._reinforce_actr_memories'), \
             patch('orchestrator.proxy_agent._calibrate_confidence', return_value=0.9), \
             patch('orchestrator.proxy_memory.resolve_memory_db_path', return_value=db_path):
...
class TestAccuracyGatesAutonomy(unittest.TestCase):
    ...
    def _calibrate(self, agent_confidence, accuracy=None, depth=5):
        ...
        from orchestrator.proxy_agent import _calibrate_confidence
        if accuracy is not None and 'last_updated' not in accuracy:
            accuracy['last_updated'] = date.today().isoformat()
        with patch('orchestrator.proxy_agent._get_memory_depth', return_value=depth):
