...
(End of Change 2 block — place a blank line after this, then ### 5.1 continues as before.)
...
Find this exact text block in Section 8.1 — a bold paragraph followed by a fenced code block followed by a trailing sentence:

Paragraph text: '**Warm-start context injection.** Before constructing the initial prompt, intent.sh must query institutional memory for observations relevant to the current task. The query takes the task description as input and returns pre-populated elements above the confidence threshold. These elements are formatted as a section in the initial prompt:'
...
Then the trailing sentence: 'This section is injected after the task description and before any other context files.'

Replace ALL of that (the bold paragraph, the code block, and the trailing sentence) with:
...
Find this exact sentence in Section 8.2 (it contains 'warmStartEnabled'):
...
Step 6a: Find the paragraph in Section 7.3 that begins: 'The primary feedback signal is delivery-point instrumentation: when the human receives the output, how much do they edit before accepting it? A high edit-to-acceptance ratio indicates the output did not reflect the intent. A low ratio indicates alignment. This signal requires no additional effort from the human and is available immediately.'
...
Find the line 'END OF DOCUMENT' at the bottom of the file. Insert the following block immediately before it, after the '---' separator that precedes END OF DOCUMENT:
...
(The above '---' is the new separator between Section 9 and END OF DOCUMENT. It should appear before the 'END OF DOCUMENT' line.)
