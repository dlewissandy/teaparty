#!/usr/bin/env python3
"""
Write proxy_wire_task.txt and then run relay.sh with the task content.
Run from: /Users/darrell/git/teaparty/poc/projects/POC
"""
import subprocess
import sys
import os

TASK = r"""Wire context-aware human proxy into shell orchestration and add test coverage.
...
# Write the task file
task_file = os.path.join(POC_DIR, "proxy_wire_task.txt")
with open(task_file, "w") as f:
    f.write(TASK)
print(f"Written task file to {task_file}", flush=True)

# Run relay.sh
result = subprocess.run(
    [os.path.join(POC_DIR, "relay.sh"), "--team", "coding", "--task", TASK],
