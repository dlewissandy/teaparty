...
def _load_project_registry(teaparty_home: str) -> dict:
    """Load unified project registry from teaparty.yaml."""
    return _load_teaparty_yaml(teaparty_home)


def _find_project_path(name: str, teaparty_home: str) -> str | None:
    """Find the absolute path for a project by name."""
    try:
        data = _load_teaparty_yaml(teaparty_home)
    except FileNotFoundError:
        return None

    repo_root = os.path.dirname(os.path.join(teaparty_home, 'management'))
...
def _load_project_yaml(project_dir: str) -> dict:
    """Load project.yaml from project's .teaparty directory."""
    paths = [
        os.path.join(project_dir, '.teaparty', 'project', 'project.yaml'),
        os.path.join(project_dir, '.teaparty', 'project.yaml'),
    ]
    for p in paths:
        if os.path.exists(p):
            with open(p) as f:
    ...
    raise FileNotFoundError(
        f'project.yaml not found in {project_dir}/.teaparty/'
    )


def _save_project_yaml(project_dir: str, data: dict) -> None:
    """Save project.yaml to the canonical location."""
    out_path = os.path.join(project_dir, '.teaparty', 'project', 'project.yaml')
