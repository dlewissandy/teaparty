...
_log = logging.getLogger('teaparty.learning.extract')
...
async def extract_learnings(
    *,
    ...
    """
    from teaparty.messaging.bus import EventType, Event

    scripts_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'episodic')
    succeeded = 0
    failed = 0
    failed_scopes = []
...
def _reinforce_retrieved(*, infra_dir: str, project_dir: str) -> None:
    ...
    from teaparty.learning.episodic.reinforce import load_ids
...
def _crystallize_skills(*, project_dir: str) -> None:
    ...
    from teaparty.learning.procedural.learning import crystallize_skills
...
def _consolidate_scope(
    ...
    *,
    ...
    """
    from teaparty.learning.consolidation import (
...
def _consolidate_proxy_memory(*, project_dir: str) -> None:
    ...
    if os.path.isfile(proxy_md_path):
        try:
            ...
            from teaparty.proxy.memory import consolidate_proxy_file
...
def _consolidate_task_learnings(*, project_dir: str) -> None:
    ...
    token similarity.
    """
    from teaparty.learning.cluster import (
...
def _compact_proxy_correction_entries(*, project_dir: str) -> None:
    ...
    from teaparty.learning.episodic.entry import parse_memory_file, serialize_memory_file
    from teaparty.learning.episodic.compact import compact_entries
    ...
    for entry in compacted:
        ...
        fpath = os.path.join(proxy_tasks_dir, fname)
        from teaparty.learning.episodic.entry import serialize_entry
        Path(fpath).write_text(serialize_entry(entry))
...
def _detect_and_write_friction(*, infra_dir: str) -> None:
    ...
    from teaparty.learning.procedural.learning import detect_friction_events
