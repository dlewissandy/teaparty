...
**No validation loop.** A learning written once and never tested has the same standing as one confirmed fifty times. The POC's early learning files contained entries that were wrong — conclusions drawn from a single session that did not generalize. Without reinforcement, decay, or contradiction detection, these persisted and actively misled subsequent sessions. The POC now addresses this: reinforcement tracking ([#91](https://github.com/dlewissandy/teaparty/issues/91)), temporal decay ([#218](https://github.com/dlewissandy/teaparty/issues/218)), and contradiction detection for proxy memory ([#228](https://github.com/dlewissandy/teaparty/issues/228)) are all operational.
...
**Confidence and decay.** Temporal decay is implemented ([#218](https://github.com/dlewissandy/teaparty/issues/218)): exponential decay with a 90-day half-life and a 10% decay floor, so old entries fade but never become invisible. Reinforcement resets the decay clock. The half-life (90 days) is a starting point — too aggressive and hard-won learnings disappear, too conservative and the store fills with stale observations. Empirical tuning is needed after real usage accumulates. Contradiction detection is implemented for proxy memory ([#228](https://github.com/dlewissandy/teaparty/issues/228)): conflicting pairs are identified by same-context divergent-outcome matching, classified by cause (preference drift, genuine tension, context sensitivity, retrieval noise) via heuristic triage then LLM-as-judge reclassification, and resolved during post-session consolidation. Contradiction detection is extended to task and institutional learnings ([#244](https://github.com/dlewissandy/teaparty/issues/244)): candidate conflicts are identified by content similarity, classified by adapted causes (temporal obsolescence, scope dependent, genuine tension, retrieval noise), and resolved during post-session consolidation. Entries involved in genuine tension have their importance reduced so they decay faster.
