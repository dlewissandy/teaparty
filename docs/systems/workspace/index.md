...
**Branch from parent, squash-merge back.** A task worktree is a branch off its job's branch. When the task completes, `teaparty/workspace/merge.py` squash-merges the task branch back into the job branch — completed work wins conflicts, because the task ran to `COMPLETED_WORK` and we do not discard finished work over mechanical merge failures. The job branch, in turn, merges back to the integration branch when the job completes. Per-repo locks in `worktree.py` serialize concurrent `git worktree add`/`remove` calls so parallel dispatches cannot race the `.git/worktrees/` advisory lock.
