...
**Reply is the recipient's final turn output.** There is no agent-facing `Reply` MCP tool. The recipient subprocess's output when its turn completes is, by convention, the reply to the opening Send. The listener captures it via `trigger_reply`, closes the child context, and delivers it to the caller. This replaced an earlier explicit Reply tool when the proxy migration to conversational prompts made structured per-turn verdicts unnecessary — the `_classify_review` script in `teaparty/cfa/actors.py` now categorizes free-text responses into CfA actions downstream.
