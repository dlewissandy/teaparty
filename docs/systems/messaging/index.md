...
**Send dispatch** (`teaparty/messaging/listener.py`, `teaparty/messaging/dispatcher.py`) runs over Unix domain sockets. When an agent calls `Send(member, message)` via MCP, the listener creates a sub-context and launches the recipient. For a **new conversation**, Send awaits the recipient to completion and returns `{status: ok, context_id, result}` with the recipient's output inline — the caller is blocked until the recipient finishes. For a **resume to an existing open conversation**, the listener returns `{status: queued}` immediately and the reply flows through asynchronously. There is no agent-facing `Reply` MCP tool — the recipient subprocess exiting IS the reply signal. When the recipient exits, `trigger_reply` closes the child context, decrements the parent's `pending_count`, injects the reply into the parent's history, and re-invokes the parent when fan-in reaches zero. A per-agent asyncio lock serializes `--resume` calls so a single agent never runs in two contexts at once.
