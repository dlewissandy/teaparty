...
Both are invisible to casual inspection. The proxy's job is to stay calibrated: escalate when it genuinely does not know, act when it genuinely does, and keep its confidence honest about which case it is in. This is the calibration challenge — every decision is also a data point about whether the proxy's confidence was warranted.
...
- Cold-start gating via ACT-R memory depth ([#220](https://github.com/dlewissandy/teaparty/issues/220))
- Contradiction detection with two-tier classification (heuristic + LLM-as-judge) ([#228](https://github.com/dlewissandy/teaparty/issues/228))
- Per-context prediction accuracy tracking per (state, task_type) ([#226](https://github.com/dlewissandy/teaparty/issues/226))
