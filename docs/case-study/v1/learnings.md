...
### Alignment observations ([tasks](./artifacts/tasks))
...
### Proxy confidence model ([proxy-confidence.json](./artifacts/proxy-confidence.json))
...
The confidence data is the system's self-assessment: it knows the TASK_ASSERT gate was where it performed worst, and the low EMA rate will inform how aggressively the proxy acts at that gate type in future sessions. However, EMA and Laplace estimates may not be the right memory model here. They decay toward a running average, not toward a principled baseline — and they treat all observations equally regardless of recency, context, or reinforcement frequency. An ACT-R activation model (see [autodiscovery.md](../../reference/autodiscovery.md#memory-model-act-r-activation)) would let frequently-reinforced patterns (like "always approve clean intents") stay active while one-off events (like "proxy timed out on this specific gate") decay naturally, without ad-hoc half-life parameters.

### Proxy behavioral patterns ([proxy-patterns.md](./artifacts/proxy-patterns.md))
...
!!! note "A note on the labels"
    ...
    This identity conflation is not intended behavior. The executing agent should know whether it is talking to the proxy or to an actual human — the two require different interaction strategies. Fixing this likely means making the proxy's identity explicit in the dialog protocol so that agents downstream in the escalation chain can distinguish proxy responses from human responses and handle timeouts accordingly.
