...
Which raises the obvious question: if the gun is pre-loaded, why do so many jokes misfire?

The hardware argument leads directly here. If the capacity for laughter is ancient and biological, installed before culture existed to have opinions about it, then jokes — which are unambiguously cultural — are something the hardware has to be loaded with. They are the software. And software, unlike hardware, has a shelf life.

Here is what running the hardware on expired software looks like.
...
Notice what this tells you. Not that laughter is culturally constructed — the laughter capacity didn't expire; the Athenian audience was laughing. Not that the joke failed because the biology was wrong — the biology was identical to yours. What expired was the cultural content loaded into the hardware. The machinery kept running. It just had nothing to shoot at.
...
The machinery ran. The machinery is still running. What changed was the software — specifically, the cultural meaning of the target. The hardware found the joke; the hardware processed the incongruity; the hardware got all the way to the point of delivery, and then the cultural layer that was supposed to say *this is contemptible* said *this is charming* instead. Same gun. Same mechanism. Different ammunition.
...
The impulse itself, however, has not gone anywhere. Rap battles work because they operate within a recognized performance frame. Everyone in the room knows the rules. Everyone knows that "going in" is theater, that the performer and the audience share a contract about the boundary between performance and genuine attack. The insult contest as competitive form appears in cultures across the world; scholars have documented some version of it on every inhabited continent. Flyting didn't die because the impulse died. It died because the specific social apparatus that held the form together — the court, the shared norms, the trained audience — dissolved. The impulse migrated to new containers and found new audiences. The hardware kept running and found new software to run.

These three cases — *The Clouds*, the *scholastikos*, flyting — are the hardware/software distinction made visible. Each one shows what happens when you isolate the variables. *The Clouds*: software fully expired, hardware running on empty, nothing produced. The *scholastikos*: hardware fully operational, mechanism firing correctly, software not expired but inverted — same joke structure, opposite emotional register, because the cultural meaning of the target flipped. Flyting: software dissolved, hardware intact, impulse decanted into new containers across centuries because the biological substrate didn't go anywhere, it just needed new material.

Laughter is older than jokes. The capacity preceded the content. When the content expires, the capacity remains. These three dead jokes are the proof.
