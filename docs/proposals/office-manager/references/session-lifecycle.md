...
The team is serialized to `.teaparty/management/agents/office-manager/.om-agents-{user}.json` and passed as `--agents <json>` to the Claude CLI. The file is cleaned up after each invocation. This means a project registered in `teaparty.yaml` is automatically included in the next OM conversation without any manual configuration.
