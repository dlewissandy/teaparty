...
Humans are team members, not external observers. This milestone builds the infrastructure for humans to participate at every level of the TeaParty hierarchy — from cross-project coordination with the office manager down to individual workgroup sessions. The system learns from their participation at each level, and what it learns at one level informs how it works at others.

For the full participation model (hierarchy, proxy transitions, cold start), see [references/participation-model.md](references/participation-model.md).
