...
The execution model is write-then-exit-then-resume. A lead that sends three parallel requests records all three outstanding threads in its conversation history before exiting its current turn. The caller is not running concurrently with its workers — it exits after `Send` and is re-invoked only when replies arrive. When workers call `Reply`, TeaParty tracks completions via the `pending_count` field in the bus context record; the lead is re-invoked when all threads close. The lead's state lives on the bus, not in process memory — durability across restarts and partial failures follows from that.
...
> **Implementation status.** `Send` and `Reply` are implemented and validated (issues #358, #359). `AskTeam` and `DispatchListener` have been retired. The write-then-exit-then-resume model is the current dispatch path. See the [conversation model](references/conversation-model.md) for context identity, multi-turn mechanics, and escalation routing.
