...
Dispatch here when requirements are unclear and work cannot begin responsibly without them. The team clarifies what is wanted, defines scope boundaries, and produces an artifact that planning and execution teams can work from. Intake is upstream of planning — dispatch it when even the approach is unknown. It is positioned to absorb the CfA intent phase as that capability matures.
...
Dispatch when the task is to define what is in and out of scope. Works from an intent summary or request and produces a scope document with explicit inclusion and exclusion statements and the reasoning behind each boundary. Not for gathering intent (intent-specialist) or interviewing stakeholders (stakeholder-interviewer).
