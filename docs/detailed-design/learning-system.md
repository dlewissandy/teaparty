...
**Full learning type differentiation.** `classify_learning_type()` maps source paths to learning categories (`institutional`, `task`, `proxy`). `retrieve()` accepts `learning_type` and `max_chars` parameters for type-aware filtering and budget caps. Session callers pass only task-based sources to `retrieve()` with `learning_type='task'`; institutional.md is loaded unconditionally via raw file read. Proxy-task retrieval is supported by the mechanism but not yet wired into callers (no `proxy-tasks/` directories exist yet). **Resolved by [#197](https://github.com/dlewissandy/teaparty/issues/197).**

**Scoped retrieval with type-aware budget allocation.** Scope multipliers are integrated (team=1.5×, project=1.2×, global=1.0×). Type-aware budget allocation is now also integrated: `retrieve()` accepts `max_chars` for per-type character budgets and `learning_type` for type filtering. Institutional memories are loaded unconditionally (always), task-based memories are fuzzy-retrieved with a dedicated budget. **Resolved by [#197](https://github.com/dlewissandy/teaparty/issues/197).**
