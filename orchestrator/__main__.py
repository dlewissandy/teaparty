...
Usage:
    python -m orchestrator "Your task description"
    python -m orchestrator --project MyProject "Your task"
    python -m orchestrator --skip-intent "Your task"
    python -m orchestrator --resume SESSION_ID_OR_PATH

Phase control:
    python -m orchestrator --intent-only "Align on this idea"
    python -m orchestrator --plan-only "Plan this feature"
    python -m orchestrator --execute-only "Just build it"

Context injection:
    python -m orchestrator --intent-file INTENT.md "Build on this"
    python -m orchestrator --plan-file PLAN.md "Execute this plan"

Testing:
    python -m orchestrator --no-human --execute-only "Fully automated"
    python -m orchestrator --dry-run -p myproject "Show memory + proxy"
    python -m orchestrator --show-memory "Show memory then run"
    python -m orchestrator --show-proxy -p myproject "Show proxy model"
    python -m orchestrator --skip-learnings "Skip learning extraction"
    python -m orchestrator -v "Verbose proxy/artifact tracing"
    python -m orchestrator --flat "No hierarchical dispatch"
...
from orchestrator.events import EventBus, EventType, InputRequest
